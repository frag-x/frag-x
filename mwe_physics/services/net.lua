...
local message_handlers = {
  ['your-id'] = function(message)
    ...
    entity_service.entities[player_id] = entity_service.spawn(player_id, x_pos, y_pos, world)
  end,
...
local event_handlers = {
  connect = function(event, is_server, world)
    ...
    if is_server then
      ...
      local player = entity_service.spawn( tostring(event.peer:connect_id()), 100, 100, world)
...
net.update = function(world)
  ...
  if event then
    ...
    event_handlers[event.type](event, net.is_server(), world)
